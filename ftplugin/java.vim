...
function! Generate_package(type, leading_package_name)
    ...
    let header = ["/\*", " * Copyright (c) 2022 Yahoo! Inc. All rights reserved.", " *\/"]
...
augroup java_make
    ...
    setlocal makeprg=set makeprg=mvn\ compile\ -q
...
" Commands to make some of the verbiage autogenerated.
augroup java_generation
    command! PrivateConstantString :normal i private static final String<Esc>
    command! PublicConstantString :normal i public static final String<Esc>
    command! PrivateConstantInteger :normal i private static final int<Esc>
    command! PublicConstantInteger :normal i public static final int<Esc>
augroup END

function! Children(className, directory)
    let command = 'grep -g *.java "(extends\|implements) ' . a:className . '" ' . a:directory
    echom(command)
    execute command
endfunction
augroup java_search
